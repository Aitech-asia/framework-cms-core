...
You can retrieve all activity using the `Core\Activities\Models\Activity` model.
...
```bash
php artisan vendor:publish --provider="Core\Activities\ActivitylogServiceProvider" --tag="migrations"
```
...
```bash
php artisan vendor:publish --provider="Core\Activities\ActivitylogServiceProvider" --tag="config"
```
...
return [
     ...
     * It should be or extend the Core\Activities\Models\Activity model.
     */
    'activity_model' => \Core\Activities\Models\Activity::class,     
];
```
