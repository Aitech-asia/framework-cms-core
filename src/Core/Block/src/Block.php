...
class Block
{
     ...
     * Constructor.
     */
    public function __construct(\Core\Block\Interfaces\CategoryRepositoryInterface $category,
        \Core\Block\Interfaces\BlockRepositoryInterface $block) {
    ...
    public function count($module)
    {
        ...
        if (User::hasRole('user')) {
            $this->block->pushCriteria(new \Core\Block\Repositories\Criteria\BlockUserCriteria());
        }
        ...
        if ($module == 'public') {
            return $this->block
                ->pushCriteria(new \Core\Block\Repositories\Criteria\BlockPublicCriteria())
                ->scopeQuery(function ($query) {
                    return $query;
                })->count();
        }
    ...
    public function getBlockCount($cid = null)
    {
        ...
        if (!empty($cid)) {
            return $this->block->pushCriteria(new \Core\Block\Repositories\Criteria\BlockPublicCriteria())
